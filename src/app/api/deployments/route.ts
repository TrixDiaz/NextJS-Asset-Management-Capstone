    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            try {
                ...
                console.log("Found storage item:", JSON.stringify(storageItem, null, 2));
                ...
                const isRequiredPartType = storageItem.subType && [ 'SYSTEM_UNIT', 'MONITOR', 'UPS' ].includes(storageItem.subType);
                ...
                if (serialRequired && !serialNumber) {
                    throw new Error(`Serial number is required for ${storageItem.subType}`);
                }
                ...
                if (serialNumber) {
                    ...
                    if (quantity > 1) {
                        throw new Error('Can only deploy one item when specifying a serial number');
                    }

                    // Remove the serial number from the storage item
                    const updatedSerialNumbers = serialNumbers.filter((s: string) => s !== serialNumber);
