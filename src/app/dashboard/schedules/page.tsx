  ...
  const filteredAndSortedSchedules = useMemo(() => {
    ...
    if (dayFilter.length > 0) {
      result = result.filter((schedule) =>
        dayFilter.includes(schedule.dayOfWeek)
      );
    }
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (schedule) =>
          ...
          (schedule.room.name &&
            schedule.room.name.toLowerCase().includes(query)) ||
          (schedule.user.firstName &&
            schedule.user.firstName.toLowerCase().includes(query)) ||
          (schedule.user.lastName &&
            schedule.user.lastName.toLowerCase().includes(query)) ||
          (schedule.user.username &&
            schedule.user.username.toLowerCase().includes(query))
      );
    }
        ...
        <div className='w-full overflow-auto'>
          {isLoading ? (
            ...
            <div className='flex flex-col items-center p-8 text-center'>
              {searchQuery || dayFilter.length > 0 ? (
                <>
                  <Calendar className='text-muted-foreground mb-4 h-16 w-16 opacity-50' />
                ...
                <>
                  <AlertCircle className='text-muted-foreground mb-4 h-16 w-16 opacity-50' />
                ...
                <TableRow>
                  <TableHead>
                    ...
                    >
                      Title
                      <ChevronDown
                        className={`ml-2 h-4 w-4 ${sortField === 'title' ? 'opacity-100' : 'opacity-50'} ${sortField === 'title' && sortDirection === 'desc' ? 'rotate-180' : ''}`}
                      />
                    </Button>
                  </TableHead>
                  <TableHead>
                    ...
                    >
                      Day
                      <ChevronDown
                        className={`ml-2 h-4 w-4 ${sortField === 'day' ? 'opacity-100' : 'opacity-50'} ${sortField === 'day' && sortDirection === 'desc' ? 'rotate-180' : ''}`}
                      />
                    </Button>
                  </TableHead>
                  ...
                  <TableHead>
                    ...
                    >
                      Room
                      <ChevronDown
                        className={`ml-2 h-4 w-4 ${sortField === 'room' ? 'opacity-100' : 'opacity-50'} ${sortField === 'room' && sortDirection === 'desc' ? 'rotate-180' : ''}`}
                      />
                    </Button>
                  </TableHead>
                  <TableHead>
                    ...
                    >
                      User
                      <ChevronDown
                        className={`ml-2 h-4 w-4 ${sortField === 'user' ? 'opacity-100' : 'opacity-50'} ${sortField === 'user' && sortDirection === 'desc' ? 'rotate-180' : ''}`}
                      />
                    </Button>
                  </TableHead>
                ...
                {paginatedSchedules.map((schedule) => (
                  <TableRow key={schedule.id}>
                    ...
                    <TableCell>
                      ...
                      {schedule.room.number || 'N/A'})
                    </TableCell>
                    <TableCell>
                      <Link
                        href={`/dashboard/users/${schedule.userId}/schedules`}
        ...
        <div className='flex items-center justify-between border-t px-4 py-4'>
          <div className='text-muted-foreground flex-1 text-sm'>
            <span className='font-medium'>
              {filteredAndSortedSchedules.length} schedules
            </span>
          </div>
