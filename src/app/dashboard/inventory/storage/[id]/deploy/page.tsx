...
export default function DeployItemPage({
  params
}: {
  ...
  const isSerialNumberRequired =
    storageItem?.itemType === 'COMPUTER_PART' &&
    storageItem?.subType &&
  ...
  const handleSerialNumberChange = (serialNumber: string, checked: boolean) => {
    if (checked) {
      ...
      const newSelected = selectedSerialNumbers.filter(
        (sn) => sn !== serialNumber
      );
      setSelectedSerialNumbers(newSelected);
      form.setValue('serialNumbers', newSelected);
  ...
  const onSubmit = async (data: DeploymentFormValues) => {
    ...
    try {
      ...
      if (
        ...
        toast.success(
          `${data.serialNumbers.length} items deployed successfully`
        );
      } else {
        // Single deployment
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error || errorData.details || 'Failed to deploy item'
          );
        }
      ...
      toast.error(
        error instanceof Error ? error.message : 'Failed to deploy item'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className='container p-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Currently in stock: {storageItem.quantity}{' '}
            {storageItem.unit || 'units'}
            {storageItem.subType && (
              <span className='ml-2'>({storageItem.subType})</span>
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {rooms.length > 0 ? (
                      ...
                      <FormItem className='flex flex-row items-start space-y-0 space-x-3 rounded-md border p-4'>
                        ...
                        <div className='space-y-1 leading-none'>
                          ...
                          <FormDescription>
                            ...
                            {storageItem.subType?.toLowerCase() || 'items'} to
                            deploy at once
                          </FormDescription>
                        </div>
                      </FormItem>
            ...
            <div className='p-4 text-center'>
              <div className='mb-4 text-amber-600'>
                No rooms available for deployment
              </div>
              <p className='mb-4'>
                Please create a room first before attempting to deploy items.
              </p>
              <div className='flex justify-center'>
                <Link href='/dashboard/inventory/seed'>
                  <Button variant='outline'>Seed Sample Data</Button>
                </Link>
              </div>
            </div>
          )}
