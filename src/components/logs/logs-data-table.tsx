...
export function LogsDataTable({ initialLogs = [] }: LogsDataTableProps) {
    ...
    const getLevelBadgeColor = (level: string): "destructive" | "secondary" | "default" | "outline" => {
        switch (level) {
            case 'error':
                return 'destructive';
            case 'warn':
    ...
    const getActionBadgeColor = (action: string): "destructive" | "secondary" | "default" | "outline" => {
        switch (action) {
    ...
    const goToPreviousPage = () => setPagination({ ...pagination, page: Math.max(1, pagination.page - 1) });
    const goToNextPage = () => setPagination({ ...pagination, page: Math.min(pagination.totalPages, pagination.page + 1) });
    const goToLastPage = () => setPagination({ ...pagination, page: pagination.totalPages });
                            ...
                            {loading ? (
                                ...
                                logs.map((log) => (
                                    <TableRow key={log.id}>
                                        ...
                                        <TableCell className="max-w-[100px] truncate" title={log.user}>
                                            {log.user}
                                        </TableCell>
                                        <TableCell>
